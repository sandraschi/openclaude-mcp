...
import { api, getCapabilities, getHealth } from "../api";
...
function useRunner() {
  const [result, setResult] = useState<ExampleResult>({
    loading: false,
    data: null,
    error: null,
    took: null,
    ...
    <div className="mt-2 rounded-lg border border-zinc-800 bg-zinc-950 overflow-hidden">
      <div className="px-3 py-1.5 border-b border-zinc-800 flex items-center gap-2 text-[10px] text-zinc-600 font-mono">
        ...
        {result.loading
          ? "Running..."
          : result.error
            ? `Error (${result.took}ms)`
            : `Success (${result.took}ms)`}
      </div>
    ...
    <div className="rounded-xl border border-zinc-800 bg-zinc-900/40 overflow-hidden">
      ...
      <div className="p-3">
        <div className="relative group">
          <pre className="font-mono text-xs text-zinc-300 bg-zinc-950 rounded-lg p-3 border border-zinc-800 overflow-auto whitespace-pre-wrap leading-relaxed pr-20">
            {code}
          </pre>
            ...
            <button
              ...
              className={`p-1.5 rounded border transition-colors text-[10px] flex items-center gap-1 ${
                result.loading
                  ...
                  : "bg-zinc-900 border-zinc-700 text-zinc-400 hover:bg-amber-500/10 hover:border-amber-500/40 hover:text-amber-300"
              }`}
            >
              {result.loading ? (
...
function StatusBar() {
  ...
  const [_cap, setCap] = useState<any>(null);
  ...
  return (
    <div className="flex items-center gap-3 flex-wrap">
      <div
        className={`flex items-center gap-1.5 px-2.5 py-1 rounded-lg border text-[10px] ${health?.ollama ? "border-emerald-500/30 bg-emerald-500/10 text-emerald-300" : "border-red-500/30 bg-red-500/10 text-red-300"}`}
      >
        <span
          className={`w-1.5 h-1.5 rounded-full ${health?.ollama ? "bg-emerald-400" : "bg-red-500"}`}
        />
        Ollama {health?.ollama ? "Connected" : "Offline"}
      </div>
      <div className="text-[10px] text-zinc-600 font-mono">
        {health?.active_sessions ?? "?"} sessions ·{" "}
        {health?.default_model ?? "?"} default
      </div>
      <button
        onClick={refresh}
...
function Step({
  ...
  return (
    <div className="rounded-lg border border-zinc-800 bg-zinc-950 overflow-hidden">
      ...
      >
        <span className="text-xs font-mono font-semibold text-amber-400 shrink-0 w-6">
          {n}
        </span>
  ...
  return (
    <div>
      <div className="relative group mt-1">
        <pre className="font-mono text-[11px] text-zinc-300 bg-zinc-900 rounded-lg p-2.5 border border-zinc-800 overflow-auto whitespace-pre-wrap leading-relaxed pr-16">
          {code}
        </pre>
        <button
          onClick={onRun}
...
export function Examples() {
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto space-y-6">
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-sm text-zinc-500 mt-0.5">
            Run real API calls against your MCP backend. Click{" "}
            <Play size={10} className="inline" /> to execute.
          </p>
        </div>
        <StatusBar />
      </div>
      ...
      {/* ── Quick Reference ──────────────────────────────────────────── */}
      <div>
        <h2 className="text-xs uppercase tracking-wider text-zinc-600 mb-3 font-medium">
          Quick Reference
        </h2>
        <div className="grid gap-3">
          <ExampleCard
            title="List Models"
            desc="gemma4:26b · qwen3.5:27b · 7 known"
...
models.known_models  // → { gemma4:26b: { ... }, ... }`}
          ...
          <ExampleCard
            title="Server Health"
            desc="Ollama status · active sessions"
        ...
        <div className="rounded-xl border border-zinc-800 bg-zinc-900/30 p-4 space-y-2">
          <Step n="1" title="Start a session with KAIROS enabled">
            <StepCard
              onRun={() =>
                sessStart.run(() =>
                  api.startSession("/tmp/demo", "gemma4:26b", true),
                )
              }
...
// → { session_id: "a1b2c3d4", status: "started" }`}
          ...
          <Step n="2" title="List all active sessions">
            <StepCard
              onRun={() => sessList.run(() => api.listSessions())}
          ...
          <Step n="3" title="Stop the session">
            <StepCard
              onRun={() => {
                ...
                if (!sid) {
                  sessStop.run(async () => {
                    throw new Error("Start a session first (Step 1)");
                  });
                  return;
                }
      ...
      <div>
        ...
        <div className="grid gap-3">
          {[
            ...
            {
              title: "list sessions",
              run: () => api.listSessions(),
              color: "text-emerald-400",
            },
            {
              title: "model status (default)",
              run: () => api.modelStatus(),
              color: "text-amber-400",
            },
            {
              title: "system logs",
              run: () => api.getSystemLogs(),
