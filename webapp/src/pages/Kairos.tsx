    ...
    <div className="p-6 max-w-4xl mx-auto">
      ...
      <div className="mb-5 rounded-xl border border-zinc-800 bg-zinc-900/40 p-4 text-sm">
        ...
        <div className="text-zinc-500 text-xs space-y-1.5">
          <div>
            <span className="text-zinc-400">Orient:</span> reads MEMORY.md from
            the working directory
          </div>
          <div>
            <span className="text-zinc-400">Gather:</span> scans daily session
            logs for new signals
          </div>
          <div>
            <span className="text-zinc-400">Consolidate:</span> calls the local
            model to merge observations, remove contradictions
          </div>
          <div>
            <span className="text-zinc-400">Prune:</span> rewrites MEMORY.md —
            vague observations become durable facts
          </div>
          <div className="pt-1 text-zinc-600">
            Triggers when the session is idle for the configured threshold. Uses
            the local Ollama model to merge observations, remove contradictions,
            and rewrite MEMORY.md. Consolidation logging is available in the
            System Logs page and below.
          </div>
        </div>
      </div>
            ...
            return (
              ...
              >
                ...
                <div className="flex items-center gap-3 px-4 py-3 border-b border-zinc-800">
                  ...
                  <button
                    onClick={() =>
                      toggleKairos(s.session_id, !s.kairos_enabled)
                    }
                  ...
                  >
                    ...
                    <span
                      className={
                        s.kairos_enabled ? "text-amber-400" : "text-zinc-600"
                      }
                    >
                ...
                {isSelected && s.kairos_enabled && (
                  <div className="p-3">
                    {logs.length === 0 ? (
                      ...
                      <div
                        className="font-mono text-xs text-zinc-400 bg-zinc-950 rounded-lg p-3
