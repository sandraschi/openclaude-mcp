import { Activity, AlertTriangle, Brain, Cpu, Terminal } from "lucide-react";
...
function StatCard({
  ...
  return (
    ...
    >
      ...
      <div>
        <div
          className={`text-2xl font-semibold ${accent ? "text-amber-300" : "text-zinc-100"}`}
        >
          {value}
        </div>
...
export function Dashboard() {
  ...
  const kairosActive = sessions.filter(
    (s) => s.kairos_enabled && s.status === "running",
  ).length;
  const available = Object.values(models).filter(
    (m) => m.available_in_ollama,
  ).length;
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto">
      ...
      {!ollamaRunning && (
        <div
          className="mb-4 flex items-center gap-3 px-4 py-3 rounded-lg border border-red-500/30
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-6">
        ...
        <StatCard
          label="Default model"
          value={
            defaultMeta?.label?.split(" ")[0] ?? defaultModel.split(":")[0]
          }
          sub={defaultModel}
          icon={Activity}
          accent
        ...
        <div className="rounded-xl border border-zinc-800 bg-zinc-900/50 p-4 mb-6">
          ...
          <div className="grid grid-cols-3 gap-3 text-xs">
            {[
              {
                k: "Active params",
                v: defaultMeta.active_params_b
                  ? `${defaultMeta.active_params_b}B`
                  : "—",
              },
              {
                k: "VRAM @ Q4",
                v: defaultMeta.vram_q4_gb
                  ? `${defaultMeta.vram_q4_gb} GB`
                  : "—",
              },
              {
                k: "Context",
                v: defaultMeta.context_k ? `${defaultMeta.context_k}K` : "—",
              },
          ...
          <div className="flex flex-col gap-2">
            {sessions.slice(0, 5).map((s) => (
              ...
              >
                ...
                />
                <span className="font-mono text-xs text-zinc-400 w-20 shrink-0">
                  {s.session_id}
                </span>
