...
# PyInstaller spec — produce openclaude-mcp-backend.exe for Tauri NSIS embedding.
# Fleet standard: strip=False, upx=False, noarchive=True (see tauri_nsis_building.md).
# Usage (from repo root):
#   uv run pyinstaller openclaude-mcp-backend.spec --distpath dist --clean --noconfirm

block_cipher = None

a = Analysis(
    ["src/openclaude/__main__.py"],
    pathex=["src"],
    binaries=[],
    datas=[("src/openclaude", "openclaude")],
    hiddenimports=[
        "uvicorn.logging",
        "uvicorn.loops",
